...
- ✅ **本地签名处理**：默认执行 ad-hoc 签名，正式分发需 Developer ID 签名和 Apple 公证
...
Azure Release 流水线会自动执行 `.app`/DMG 生成步骤：macOS 架构 ZIP 的顶层内容是对应的 `.app`，并额外发布 Universal `.app` 压缩包和 DMG。终端用户不需要自行运行这些构建命令。
...
发布前应检查 NuGet 安全警告并升级存在已知漏洞的依赖，不应忽略高严重性警告。
