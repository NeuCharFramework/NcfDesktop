...
### 🎙️ **AdminChat 本地语音输入**
- 可从 AdminChat 输入区或桌面机器人直接开始/停止录音
- 首次使用前必须在“工作台设置 → 本地语音输入”选择模型；缺少内置模型时会提示下载
- 支持 Tiny、Base、Small 多语言 Whisper 模型，也可手动加载本地 whisper.cpp GGML `.bin` 模型
- 音频只在内存中交给本机 Whisper 转写，不上传；识别结果先进入输入框，确认后才发送
- 已安装模型可离线使用；内置模型首次下载需要网络
- 支持自定义默认唤醒词和多个唤醒词；每个唤醒词可关联到指定 AdminChat 会话，不同唤醒词可以进入不同任务上下文
- 默认使用中文 KWS 模型，中文唤醒词会自动生成当前模型需要的拼音；切换到“中文 + English”模型后，可使用中文、英文和中英混合唤醒词
- 双语模型根据本地英文发音词典生成英文音素；不支持的英文单词会单独标记并跳过，不影响其他唤醒词
- 唤醒检测使用多关键词、INT8、单线程 sherpa-onnx 流式模型；只在 AdminChat 与语音模型可用时监听，录音和转写期间自动暂停；Agent 正在回复时仍可唤醒，转写结果会排队发送
- 未唤醒音频只在内存中流过，不保存、不上传；关闭唤醒后立即释放麦克风
- 唤醒后会异步播放随机确认词（如“我在”“收到”或 “Acknowledged”“Roger that”），不会阻塞录音、转写或消息发送

### 🔊 **AdminChat 本地流式朗读**
- 使用本地 sherpa-onnx Kokoro 模型，可手动朗读单条 AI 回复，也可开启自动朗读
- 自动朗读直接消费 Admin Chat SSE token；首个完整短句就绪后即可开始，无需等待整篇回答结束
- Markdown、链接和代码块会在完整边界处统一清理，避免朗读半截标记、URL 或代码正文
- Kokoro 生成的 PCM 分片持续写入同一个播放队列；整次回复复用音频设备，并在播放上一片时继续合成下一片
- 新的朗读、语音输入、停止操作或请求失败都会取消当前流式会话，最终消息到达后不会重新从头朗读

## 🚀 **使用方法**

### 启动应用程序
```bash
cd src/NcfDesktopApp.GUI
dotnet run
```
...
- **跨平台文件管理**：智能路径处理
- **端口管理**：自动端口检测和分配
- **SoundFlow + Whisper.net**：跨平台麦克风采集与本地离线语音转写
- **sherpa-onnx KWS**：单关键词 INT8 本地流式唤醒检测，不使用 Whisper 常驻转写
- **sherpa-onnx Kokoro + SoundFlow**：本地 PCM 流式合成、队列播放与音频可视化
...
- **框架依赖发布包**：需要 .NET 8.0 Runtime
- **源码开发和运行**：建议使用 .NET 10.0 SDK（应用目标运行时为 net8.0，以兼容当前 macOS WebView 原生桥）
...
1. **应用程序无法启动**
   ...
   - 框架依赖版本需安装 .NET 8.0 Runtime；正式自包含版本无需安装
   - 检查系统兼容性
